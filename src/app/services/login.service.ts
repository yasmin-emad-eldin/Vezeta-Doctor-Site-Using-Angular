...
//    return   this.http.post<any>(`${environment.APIBaseURL1}/angular-doctor-login`, payload, { observe: 'response'})
//   //   // return user;
...
userLogin1(payload:any):Observable<any>{
  ...
  adminProfile1():Observable<any>{
  this. y=localStorage.getItem("AdminToken5")

    let headers = new HttpHeaders()
     .set('doctor-token',this.y)
     let x=localStorage.getItem("AdminId5")
   return this.http.get<any>(`${environment.APIBaseURL1}/doctor-profile/${x}`,{'headers':headers})
    }
}
